...
/* 00. Program Set Up */
    ...
    cap log using "$root/_log/A1_complete_v`version'_`datetime'.log", replace
...
if `part1' == 1{
    ...
    /* 09. Age first observed */
        ...
        local i = 1
        while `i' <= `: word count `ages'' {
            local var : word `i' of `ages'
             replace age_first_observed = `var' if `var' != . & (age_first_observed == . | `var' < age_first_observed)
            local i = `i' + 1
        }
    ...
    /* 10. Waves & waves under 18 */
        replace in_1968 = 0 if in_1968 == .
        replace in_1969 = 0 if in_1969 == .
        replace in_1970 = 0 if in_1970 == .
        replace in_1971 = 0 if in_1971 == .
        replace in_1972 = 0 if in_1972 == .
        replace in_1973 = 0 if in_1973 == .
        replace in_1974 = 0 if in_1974 == .
        replace in_1975 = 0 if in_1975 == .
        replace in_1976 = 0 if in_1976 == .
        replace in_1977 = 0 if in_1977 == .
        replace in_1978 = 0 if in_1978 == .
        replace in_1979 = 0 if in_1979 == .
        replace in_1980 = 0 if in_1980 == .
        replace in_1981 = 0 if in_1981 == . 
        replace in_1982 = 0 if in_1982 == .
        replace in_1983 = 0 if in_1983 == .
        replace in_1984 = 0 if in_1984 == .
        replace in_1985 = 0 if in_1985 == .
        replace in_1986 = 0 if in_1986 == .
        replace in_1987 = 0 if in_1987 == .
        replace in_1988 = 0 if in_1988 == .
        replace in_1989 = 0 if in_1989 == .
        replace in_1990 = 0 if in_1990 == .
        replace in_1991 = 0 if in_1991 == .
        replace in_1992 = 0 if in_1992 == .
        replace in_1993 = 0 if in_1993 == . 
        replace in_1994 = 0 if in_1994 == .
        replace in_1995 = 0 if in_1995 == .
        replace in_1996 = 0 if in_1996 == .
        replace in_1997 = 0 if in_1997 == .
        replace in_1999 = 0 if in_1999 == .
        replace in_2001 = 0 if in_2001 == .
        replace in_2003 = 0 if in_2003 == .
        replace in_2005 = 0 if in_2005 == .
        replace in_2007 = 0 if in_2007 == .
        replace in_2009 = 0 if in_2009 == .
        replace in_2011 = 0 if in_2011 == .
        replace in_2013 = 0 if in_2013 == .
        replace in_2015 = 0 if in_2015 == .
        replace in_2017 = 0 if in_2017 == .
        replace in_2019 = 0 if in_2019 == .
        replace in_2021 = 0 if in_2021 == .
        replace in_2023 = 0 if in_2023 == .
    ...
    /* 11. Qualified family 
        * these are the families i will pull year to year data on, who are home to at least one person
        observed at least twice in the psid before the age of 18
        right now, before drop, n=85836 people 
        */
        g qualified_family = 1 if waves_18_under >= 2
        replace qualified_family = 0 if waves_18_under == 0
        egen qfam = max(qualified_family), by(fam)
        replace qualified_family = qfam
        drop qfam
        label var qualified_family "Qualified family (observed at least twice in PSID at or before age 18)"
        
    /* 12. Drop unqualified families */
        drop if qualified_family == 0
        * n = 77,505 people remain, 44,972 of them are the individuals observed at least twice under 18, 
        * rest are family members
    ...
    /* 13. FIMS 
        ...
        * Merge on grandparents FIMS file (note--this also has the parent variables)
        ...
        merge 1:1 ID using "${root}/_fims_gpars_clean.dta"
        * drop people not in individual file 
        drop if _merge == 2 
        * 77,505 people
        drop _merge

        * Merge on siblings FIMS file
        merge 1:1 ID using "${root}/_fims_sib_clean.dta"
        * 78,256 people, 40,036 of them kids




    /* 14. RESHAPE LONG */ 
        reshape long age_, i(ID) j(year)
        order ID fam person_number age_first_observed
        * 3,365,008 person-year observations, 
        * 78,256 people
        * 40,036 of them sample members

    /* 25. **Reduce file size */
        * drop people from gpars or sib files who aren't in the individual file
        drop if _merge == 2 
        * 3,332,715 person-year observations
        * 77,505 people
        * 39,285 of them sample members


    /* 16. Identify SPLITOFF FAMILIES */
    ...
    /* 19. THE BIG TRIM */
        * drop rows when no one in family was interviewed in that year
        drop if family_year_id == 0
        * 2,063,023 person-years
        * 77,505 people
        * 39,285 of them sample members observed at least twice at or before age 18

        * drop if age is missing AND IT IS NOT THE LAST OBSERVATION FOR 
        * THAT PERSON (dropping empty rows before someone is first observed
        * but keeping one empty row after the last observation for that person)
        sort ID yr
        drop if age_ == . & ID == ID[_n+1]
        * now: 883,068 person-years, 
        * 77,505 people
        * 39,285 of them sample members observed at least twice at or before age 18 
    ...
    /* 20. Household Roster */
        ...
        * without self
        ...
        replace hhr = "" if age_ == .
        replace hhr_no_self = "" if age_ == .

    /* 21. save and export */
        drop _merge
        save "$root/_psid_long.dta", replace
...
if `part3' == 1{
    ...
    /* 01. Merge on */
        use "_heads_panel.dta", clear
        cap drop _merge
        merge 1:m ID yr using "$root/_psid_long.dta"
        sort ID yr 
        * 1,133,245 person-years
        * 90,593 people
        drop if family_year_id == . 
        * 883,068 person-years
        * 80,814 people
        * 39,286 sample members
        * one straggler ? 
        drop if qualified_family == .

    /* 02. Checkpoint April 8 before dropping for meeting */
        tempfile april8 
        save `april8', replace
    ...
    *  39,285 in sample
    ...
    /* 20. Ages Roster */
        ...
        * for each year in each family, replace hhr with a list of the IDs of the family members with the same value of family_year_ID in that year. 
        ...
        bysort fam yr family_year_id (ID): replace ages_hhr = str_age[1]
        bysort fam yr family_year_id (ID): replace ages_hhr = ages_hhr[_n-1] + " " + str_age if _n > 1
        bysort fam yr family_year_id (ID): replace ages_hhr = ages_hhr[_N]
        drop str_age
        replace ages_hhr = "" if kid_sample == .


    /* 21. Relationships Roster */
        g rel_hhr = ""

        sort fam yr family_year_id ID

        * for each year in each family, replace hhr with a list of the IDs of the family members with the same value of family_year_ID in that year. 
        g str_rel = string(head_rel)
        bysort fam yr family_year_id (ID): replace rel_hhr = str_rel[1]
        bysort fam yr family_year_id (ID): replace rel_hhr = rel_hhr[_n-1] + " " + str_rel if _n > 1
        bysort fam yr family_year_id (ID): replace rel_hhr = rel_hhr[_N]
        drop str_rel
        replace rel_hhr = "" if kid_sample == .
