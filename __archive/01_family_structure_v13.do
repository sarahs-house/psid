...
/* 00. Program Set Up */
    ...
    log using "$root/_log/01_family_structure_v13_`datetime'.log", replace
    ...
    * Switches
    ...
    local qualified "$root/01_qualified_families_v13.dta"
...
if `part1' == 1 {
    ...
    /* 02. Generate Variables and observe */
        ...
        * DROP ANY KIDS NOT OBSERVED between 0 and 16
        egen min_age = min(age), by(ID)
        drop if min_age != 1


    /* 03. Drop observations after age 16 */
        drop if age > 16

        * Drop any kids who we only observe once in childhood
        bysort ID: g wave_count =  _n
        egen max_wave = max(wave_count), by(ID)
        drop if max_wave == 1
        drop max_wave wave_count

    /* 03a. Mom marital birth */
    ...
    /* 12. Save family composition file */
        save "$root/01_family_structure_v13.dta", replace
        ...
        save "$root/01_qualified_kids_v13.dta", replace
...
if `part2' == 1{
    /* 13. Individual file */
        ...
        merge 1:m ID using "01_qualified_kids_v13.dta"
    ...
    /* 24. Save and export to python */
        ...
        save `temp2'
        label drop _all
        save "$root/01_try_pyth_v13.dta", replace
...
if `part3' == 1{
    ...
    * I do this in python file, final_data_v3.ipynb * 
    clear
    import delimited "final_data_v3.csv"
    save "$root/01_hhr_v13.dta", replace
    rename id ID 
    collapse ID, by(fam)
    drop ID 
    save "$root/01_qualified_families_v13.dta", replace
}
...
if `part4' == 1{
    ...
    /* 22. Merge on Household Roster Data */
        append using "$root/01_hhr_v13.dta"
    ...
    /* 25. Save */
        save "$root/01_heads_kids_panel_v13.dta", replace
}
...
if `part5' == 1{
    ...
    /* 26. Merge on individual data */
        ...
        merge 1:m ID using "$root/01_heads_kids_panel_v13.dta", generate(new_merge)
        drop if yr == .
