...
/* 00. Program Set Up */
    ...
    log using "$root/_log/01_family_structure_v12_`datetime'.log", replace
    ...
    * Switches
    ...
    local qualified "$root/01_qualified_families_v12.dta"
...
if `part1' == 1 {
    ...
    /* 02. Generate Variables and observe */
        ...
        * DROP ANY KIDS NOT OBSERVED AT LEAST ONCE IN CHILDHOOD (BEFORE 16)
        bysort ID: egen min_age_observed = min(Age)
        drop if min_age_observed > 16
        drop min_age_observed

    /* 03. Population born between 1960 and 2000 */
        * Drop anyone born after 2000 --> give them a chance to become an adult
    ...
    /* 11. Year Child first/last observed */
        ...
        * DECEMBER 13--> DO I NEED THESE ? LET'S DROP FOR NOW
        drop bio_md_together bio_md_ever_together bio_mom_single bio_dad_single a_mom_single a_dad_single

    /* 12. Save family composition file */
        save "$root/01_family_structure_v12.dta", replace
        collapse (firstnm) mom_marital_birth (mean) fam fam_number birth_year bio_mom_id bio_dad_id a_mom_id a_dad_id kid_sample yr_first_observed yr_last_observed, by(ID)
        save "$root/01_qualified_kids_v12.dta", replace
...
if `part2' == 1{
    /* 13. Individual file */
        ...
        merge 1:m ID using "01_qualified_kids_v12.dta"
    ...
    /* 22. Drop */
        ...
        * DROP ANY KIDS NOT OBSERVED AT LEAST ONCE IN ADULTHOOD (AFTER 21)
        bysort ID: egen max_age_observed = max(Age)
        drop if max_age_observed < 21
    ...
    /* 25. Save and export to python */
        tempfile temp2
        save `temp2'
        label drop _all
        save "$root/01_try_pyth_v12.dta", replace
...
if `part3' == 1{
    ...
    * I do this in python file, final_data_v1.ipynb * 
    clear
    import delimited "final_data_v2.csv"
    save "$root/01_hhr_v12.dta", replace
    rename id ID 
    collapse ID, by(fam)
    drop ID 
    save "$root/01_qualified_families_v12.dta", replace
}
...
if `part4' == 1{
    ...
    /* 21. Merge together */
        ...
        *append using "$root/fam2021er/fam2021_qualifiedsample.dta"
        ...
        save "$root/01_heads_panel_v12.dta", replace

    /* 22. Merge on Household Roster Data */
        append using "$root/01_hhr_v12.dta"
    ...
    /* 25. Save */
        save "$root/01_heads_kids_panel_v12.dta", replace
}
...
if `part5' == 1{
    ...
    /* 26. Merge on individual data */
        ...
        merge 1:m ID using "$root/01_heads_kids_panel_v12.dta", generate(new_merge)
        drop if yr == .
...
if `part6' == 1{
    drop if kid_sample != 1
    drop ER* qualified_family kid_sample is_first_obs
