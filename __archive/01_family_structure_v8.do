...
/* 00. Program Set Up */
    ...
    log using "$root/_log/01_family_structure_v8_`datetime'.log", replace
...
/* 12. Save family composition file */
    save "$root/01_family_structure_v8.dta", replace
    ...
    save "$root/01_qualified_kids_v8.dta", replace
...
/* 13. Individual file */
    ...
    merge 1:m ID using "01_qualified_kids_v8.dta"
...
/* 19. Save hh roster time series */
        ...
        save "$root/01_hhr_v8.dta", replace
        collapse ID, by(fam)
        drop ID 
        save "$root/01_qualified_families_v8.dta", replace


/* ------------------------------------- */
* PART II: merge on data from each survey 
* wave
/* ------------------------------------- */

    local qualified "$root/01_qualified_families_v8.dta"
    use "`qualified'"
...
/* 21. Merge together */
    ...
    *append using "$root/fam2021er/fam2021_qualifiedsample.dta"
    ...
    save "$root/01_heads_panel_v8.dta", replace

/* 22. Merge on Household Roster Data */
    append using "$root/01_hhr_v8.dta"
...
/* 24. Save final dataset */
    save "$root/01_heads_kids_panel_v8.dta", replace
   
   
* STOPPED OCTOBER 24 9:22PM
    * go back and make a variable for ages of people in HHR (added/removed)
        * identity --> relationship to kid_in_sample --> derive from relationship to head and then relationship to kid matrix? 
        * dummy for if they lost/gained an adult
        * dummy for if they lost/gained a kid
        * race, gender, mom's education
    * income change variable for kids in sample based on if there was a change in hhr for kids lt18
        * FU income in year-1 when there was a change in hhr for kids lt18
        * FU income in year-1 when there was NO change in hhr for kids lt18
    * by age at time of change (entry/exit)

/* 25. Merge individual data on */
 keep Age kid_sample fam ID yr FU_income hhr_ fam_number birth_year yr_first_observed yr_last_observed change_in_hhr hhr_added_count hhr_removed_count hhr_added hhr_removed change_hhr_0_18 hhr_added_0_18 hhr_removed_0_18 hhr_added_count_0_18 hhr_removed_count_0_18 FU_income_change FU_income_no_change
