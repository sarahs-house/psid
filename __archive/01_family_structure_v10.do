...
/* 00. Program Set Up */
    ...
    log using "$root/_log/01_family_structure_v10_`datetime'.log", replace
...
/* 12. Save family composition file */
    save "$root/01_family_structure_v10.dta", replace
    ...
    save "$root/01_qualified_kids_v10.dta", replace
...
/* 13. Individual file */
    ...
    merge 1:m ID using "01_qualified_kids_v10.dta"
...
/* 17. Age */
    ...
    save "$root/01_try_pyth_v10.dta", replace
...
/* 18. Identify changes in hhr */
    * I do this in python file, final_data_v1.ipynb * 
    ...
    save "$root/01_hhr_v10.dta", replace
    rename id ID 
    collapse ID, by(fam)
    drop ID 
    save "$root/01_qualified_families_v10.dta", replace
...
/* ------------------------------------- */
    ...
    local qualified "$root/01_qualified_families_v10.dta"
    use "`qualified'"
...
/* 21. Merge together */
    ...
    *append using "$root/fam2021er/fam2021_qualifiedsample.dta"
    ...
    save "$root/01_heads_panel_v10.dta", replace

/* 22. Merge on Household Roster Data */
    append using "$root/01_hhr_v10.dta"
...
/* 24. Save final dataset */
    save "$root/01_heads_kids_panel_v10.dta", replace
...
/* 26. Merge on individual data */
    ...
    merge 1:m ID using "$root/01_heads_kids_panel_v10.dta", generate(new_merge)
    drop if yr == .
...
/* 27. Clean up  */
    ...
    save "$root/01_kids_panel_v10.dta", replace
